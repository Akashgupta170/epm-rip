...
function AddAccessories() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/getaccessorycategory', {
        headers: {
  ...
  const handleSubmit = async (formData, id) => {
    const url = id
      ? `http://127.0.0.1:8000/api/updateaccessorycategory/${id}`
      : 'http://127.0.0.1:8000/api/addaccessorycategory';
  ...
  const handleEditClick = async (id) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/editaccessorycategory/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteClick = async (id) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8000/api/deleteaccessorycategory/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="w-full bg-white shadow rounded-2xl">
      <SectionHeader icon={BarChart} title="Accessories" subtitle="View, edit and manage accessories" />
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex justify-between items-center flex-wrap gap-4">
          ...
          <input type="search" placeholder="Search accessories..." className="ml-auto border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
      </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="border-b border-gray-800 bg-blue-600 text-white">
              <tr>
                ...
                <th className="px-4 py-2 text-center">Accessory Name</th>
