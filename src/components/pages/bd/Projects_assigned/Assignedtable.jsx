...
function ProjectCard({ project, editProjectId, editProjectName, setEditProjectName, handleEditClick }) {
  ...
  const handleRemoveManagers = async () => {
    if (selectedManagers.length === 0) return alert("Select at least one manager.");

    const result = await removeProjectManagers(project.id, selectedManagers);

    if (result.success) {
      alert("Managers removed successfully.");
      setShowRemoveList(false);
      setSelectedManagers([]);

    } else {
      alert(result.message || "Failed to remove managers.");
    }
  };
        ...
        <div className="flex items-center justify-between text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
          ...
          {Array.isArray(project.project_managers) &&
            project.project_managers.some((pm) => pm.id !== null) && (
              ...
              //   className="bg-yellow-500 text-white px-3 py-1 rounded-lg text-xs font-medium shadow hover:bg-yellow-600 transition flex items-center gap-1"
              // >
